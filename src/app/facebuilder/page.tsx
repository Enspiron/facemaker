...
type TrimEntry = [string, string, string, string]; // [x, y, canvasW, canvasH]
...
function imgProxy(cdnUrl: string) {
  return `/api/assets/image?url=${encodeURIComponent(cdnUrl)}`;
}

function thumbnailUrl(face: string) {
  return imgProxy(`${CDN}/${face}/ui/battle_member_status_0.png`);
}

function storyUrl(face: string, stem: string) {
  return imgProxy(`${CDN}/${face}/ui/story/${stem}.png`);
}
...
function resolveBase(baseFiles: string[], variant: "0" | "1"): string | null {
  ...
  const find = (name: string) => (baseFiles.includes(name) ? name : null);
  const resolved =
    variant === "0"
...
function CharacterList({
  ...
  playableOnly, setPlayableOnly, selectedFace, onSelect,
}: {
  faces: string[];
  faceUi: Record<string, FaceUiData>;
  nameMap: Record<string, string>;
  search: string;
  setSearch: (v: string) => void;
  playableOnly: boolean;
  setPlayableOnly: (v: boolean) => void;
  selectedFace: string | null;
  onSelect: (face: string) => void;
}) {
  const filtered = faces.filter((face) => {
    const data = faceUi[face];
    if (playableOnly && !data?.ui.files.includes("battle_member_status_0.png")) return false;
    if (search) {
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="p-3 border-b space-y-2 shrink-0">
        ...
        <Button
          variant={playableOnly ? "default" : "outline"}
          size="sm"
          className="w-full h-7 text-xs"
          onClick={() => setPlayableOnly(!playableOnly)}
        >
          {playableOnly ? "Playable only" : "All characters"}
        </Button>
      </div>
          ...
          return (
            <button
              ...
              className={`w-full flex items-center gap-2 px-3 py-1.5 text-left hover:bg-accent transition-colors ${
                selectedFace === face ? "bg-accent" : ""
              }`}
            >
              {isPlayable ? (
                // eslint-disable-next-line @next/next/no-img-element
                <img
                  src={thumbnailUrl(face)}
                  alt=""
              ...
              {isPlayable && (
                <Badge variant="secondary" className="text-[9px] px-1 shrink-0">PC</Badge>
              )}
            </button>
          );
        })}
...
function ExpressionControls({
  ...
  selectedFace: string;
  baseVariant: "0" | "1";
  setBaseVariant: (v: "0" | "1") => void;
  hasVariant1: boolean;
  expressionFiles: string[];
  otherPartFiles: string[];
  selectedExpr: string | null;
  setSelectedExpr: (v: string | null) => void;
  otherParts: Set<string>;
  setOtherParts: (fn: (prev: Set<string>) => Set<string>) => void;
  onReset: () => void;
}) {
  return (
          ...
          <div className="grid grid-cols-4 md:grid-cols-3 gap-1">
            {expressionFiles.map((expr) => (
              <button key={expr}
                ...
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img src={storyUrl(selectedFace, expr)} alt={expr}
                  className="w-full h-full object-cover"
                  onError={(e) => ((e.target as HTMLImageElement).style.opacity = "0.2")} />
                <span className="absolute bottom-0 left-0 right-0 text-[8px] text-center bg-black/50 text-white px-0.5 truncate">
                  {expr}
                </span>
              </button>
            ))}
          ...
          <div>
            ...
            <div className="grid grid-cols-4 md:grid-cols-3 gap-1">
              {otherPartFiles.map((part) => {
                ...
                return (
                  <button key={part}
                    onClick={() => setOtherParts((prev) => {
                      const next = new Set(prev);
                      active ? next.delete(part) : next.add(part);
                      return next;
                    })}
                    className={`relative rounded border overflow-hidden aspect-square transition-all ${
                    ...
                    {/* eslint-disable-next-line @next/next/no-img-element */}
                    <img src={storyUrl(selectedFace, part)} alt={part}
                      className="w-full h-full object-cover"
                      onError={(e) => ((e.target as HTMLImageElement).style.opacity = "0.2")} />
                    <span className="absolute bottom-0 left-0 right-0 text-[8px] text-center bg-black/50 text-white px-0.5 truncate">
                      {part}
...
export default function FacebuilderPage() {
  ...
  const [baseVariant, setBaseVariant] = useState<"0" | "1">("0");
  const [selectedExpr, setSelectedExpr] = useState<string | null>(null);
  const [otherParts, setOtherParts] = useState<Set<string>>(new Set());
  ...
  useEffect(() => {
    async function init() {
      ...
      for (const [charId, vals] of Object.entries(charData)) {
        const faceKey = vals[0];
        const name = charTextData[charId]?.[0];
        if (faceKey && name) names[faceKey] = name;
      }
  ...
  function handleSelectFace(face: string) {
  ...
  const charListProps = {
    faces, faceUi, nameMap, search, setSearch,
    playableOnly, setPlayableOnly, selectedFace,
  ...
  const exprControlProps = selectedFace ? {
    selectedFace, baseVariant, setBaseVariant, hasVariant1,
    expressionFiles, otherPartFiles, selectedExpr, setSelectedExpr,
    otherParts, setOtherParts, onReset: handleReset,
  } : null;
  ...
  return (
    <div className="flex h-dvh overflow-hidden bg-background">
      ...
      {/* Character list — desktop sidebar */}
      ...
      {/* Expression controls — desktop sidebar */}
      ...
      {/* Main area — full width on mobile, flex-1 on desktop */}
      <div className="flex flex-col flex-1 min-w-0">
        ...
        {/* Canvas area — shared between mobile and desktop */}
          ...
          {!selectedFace ? (
            ...
            <>
              ...
              <div className="relative rounded-lg border bg-background shadow-sm overflow-hidden w-full md:w-auto">
                ...
                <canvas
                  ref={canvasRef}
                  className="block max-w-full max-h-[55dvh] md:max-h-[70vh] mx-auto"
